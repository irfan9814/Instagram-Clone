...
export const createComment = (data) => async (dispatch) => {
  ...
  const res = await fetch(`http://localhost:8080/api/comments/create/${data.postId}`, {
    method: "POST",
...
export const findPostComment=(data)=>async(dispatch)=>{
    const res= await fetch(`http://localhost:8080/api/comments/${data.postId}`,{
        method:"GET",
...
export const likeComment=(data)=>async(dispatch)=>{
    const res= await fetch(`http://localhost:8080/api/comments/like/${data.commentId}`,{
        method:"PUT",
        headers:{
            "Content-Type":"application/json",
            Authorization:'Bearer '+data.jwt,
...
export const unLikeComment=(data)=>async(dispatch)=>{
  const res = await fetch(`http://localhost:8080/api/comments/unlike/${data.commentId}`,{
      method:"PUT",
      headers:{
          "Content-Type":"application/json",
          Authorization:'Bearer '+data.jwt,
